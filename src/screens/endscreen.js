...
app.controller("LoadingCtrl", [
  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    $scope.getControllerTranslations = function () {
      translationsAvailable().then(() => {
        $translate([
          ...
          $scope.detail.shutdown1 =
            $scope.ctrlTranslations["endscreen.shutdown1"] ||
            "GemCore is shutting down";
          $scope.detail.shutdown2 =
            $scope.ctrlTranslations["endscreen.shutdown2"] ||
            "Please do not shutdown the computer";
          $scope.detail.shutdown3 =
            $scope.ctrlTranslations["endscreen.shutdown3"] ||
    ...
    function checkDaemon() {
      ...
      if (index > -1) {
        port =
          $scope.settings.confData.rpcport != undefined
            ? $scope.settings.confData.rpcport
            : $scope.settings.serverData.testnet_rpcport;
      } else {
        port =
          $scope.settings.confData.rpcport != undefined
            ? $scope.settings.confData.rpcport
            : $scope.settings.serverData.rpcport;
      }
      var methods = "getinfo";
      var arg = [];
    ...
    function stopDaemon() {
      ...
      if (
        $scope.settings == null ||
        $scope.settings.confData == null ||
        $scope.settings == undefined ||
        $scope.settings.confData == undefined
      ) {
        setTimeout(closeApp, 2000);
      } else {
        if (index > -1) {
        ...
        curlData(
          $scope.settings.confData.rpcuser,
          $scope.settings.confData.rpcpassword,
          port,
          methods,
          arg
        );
