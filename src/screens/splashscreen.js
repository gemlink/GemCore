...
app.controller("SplashCtrl", [
  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    $scope.detail.progressbarWidth =
      remote.getCurrentWindow().getBounds().width / 2;
    ...
    $scope.applyLocationAction = function () {
      if (
        $scope.detail.daemonLocation == "" ||
        $scope.detail.daemonLocation == undefined
      ) {
        spawnMessage(
          MsgType.ALERT,
          $scope.ctrlTranslations["splashscreen.daemonLocation"]
        );
      } else {
        ipc.send("main-update-data-loading", {
          type: UpdateDataType.APPLYLOCATION,
          data: $scope.detail.daemonLocation,
        });
      }
    };
    ...
    $scope.cancelLocationAction = function () {
      spawnMessage({
        type: MsgType.AUTO_DOWNLOAD_DAEMON,
        text: $scope.ctrlTranslations["splashscreen.autoDaemonDownload"],
        title: "",
      });
    };
    ...
    $scope.selectLinux = function (version) {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.SELECTLINUX,
        data: version,
      });
    };
      ...
      dialog.showOpenDialog(dialogOptions, function (fileNames) {
        $timeout(function () {
          ...
          if (fileNames === undefined) {
            ...
            document.getElementById("daemon-file").value =
              $scope.detail.daemonLocation = fileNames[0];
          }
        }, 0);
      });
    };
        ...
        $timeout(function () {
          ...
          if (folder === undefined) {
            ...
            document.getElementById("data-dir").value =
              $scope.detail.dataLocation = folder[0];
          }
        }, 0);
      });
    };
    ...
    $scope.applyCustomDataAction = function () {
      if (
        $scope.detail.dataLocation == "" ||
        $scope.detail.dataLocation == undefined
      ) {
        spawnMessage({
          type: MsgType.ALERT,
          text: $scope.ctrlTranslations["splashscreen.dataFolder"],
        });
      } else {
        ipc.send("main-update-data-loading", {
          type: UpdateDataType.APPLYCUSTOMDATA,
          data: $scope.detail.dataLocation,
        });
      }
    };
    ...
    $scope.canceCustomDataAction = function () {
      spawnMessage({
        type: MMsgType.CUSTOM_DATA,
        text: $scope.ctrlTranslations["splashscreen.defaultDataLocation"],
        title: "",
      });
    };
    ...
    $scope.autoDownloadBlockchain = function (value) {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.AUTODOWNLOADBLOCKCHAIN,
        data: value,
      });
    };
    ...
    $scope.dependenciesAction = function () {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.DEPENDENCIESACTION,
      });
    };

    $scope.daemonAction = function () {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.ERRORDAEMON,
        data: $scope.detail.text,
      });
    };
        ...
        $timeout(function () {
          $scope.detail.title =
            data.title == undefined
              ...
              : data.title;
          $scope.detail.text = typeof data.text == 'string' ? data.text : JSON.stringify(text, null, 2);
      ...
      function downloadFileInside(file_url, targetPath) {
        ...
        req.on("error", function (err) {
          ...
          spawnMessage({
            type: MsgType.ALERT,
            text:
              $scope.ctrlTranslations["splashscreen.downloadErr1"] ||
              "Downloading error, restarting wallet",
          });
        });
            ...
            if (received_bytes == total_bytes) {
              ...
              unzipper.on("error", function (err) {
                $timeout(function () {
                  ...
                  spawnMessage({
                    type: MsgType.ALERT,
                    text:
                      ($scope.ctrlTranslations["splashscreen.downloadErr2"] ||
                        "Download files error, please restart wallet") +
                      " " +
                      err.message,
                  });
                  $scope.detail.disableProgressbar = true;
              ...
              unzipper.on("extract", function (log) {
                $timeout(function () {
                  ...
                  if (
                    data.step == Steps.DOWNLOAD_PARAMS ||
                    data.step == Steps.DOWNLOAD_DAEMON
                  ) {
                    fs.unlinkSync(saveFile);
                    ipc.send("main-update-data-loading", {
                      type: UpdateDataType.CHECKDAEMON,
                    });
              ...
              unzipper.on("progress", function (fileIndex, fileCount) {
                $timeout(function () {
                  var percentage = (
                    (parseFloat(fileIndex + 1) / fileCount) *
                    100
                  ).toFixed(2);
                  updateScope(
                    ($scope.ctrlTranslations["splashscreen.scopeUpdate1"] ||
        ...
        function showProgress(received, total) {
          ...
          $timeout(function () {
            updateScope(
              ($scope.ctrlTranslations["splashscreen.scopeUpdate2"] ||
                "Downloading") +
                " " +
                data.name +
                " (" +
                (received / 1024 / 1024).toFixed(2) +
                "MB/ " +
    ...
    $scope.selectCoin = function (coin) {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.SELECTCOIN,
        data: coin,
      });
    };
    ...
    $scope.selectCoinSettings = function (coin) {
      ipc.send("main-update-data-loading", {
        type: UpdateDataType.SELECTCOINSETTINGS,
        data: coin,
      });
    };
    ...
    $scope.autoDownload = function (value) {
      if (value == true) {
        ipc.send("main-update-data-loading", {
          type: UpdateDataType.AUTODOWNLOAD,
          data: true,
        });
      } else {
        spawnMessage({
          type: MsgType.DAEMON_LOCATION,
          text: undefined,
          btnText: undefined,
          title: $scope.ctrlTranslations["loadingView.selectBinaryFiles"],
        });
      }
    };
    ...
    $scope.customData = function (value) {
      if (value == false) {
        ipc.send("main-update-data-loading", {
          type: UpdateDataType.CUSTOMDATA,
          data: false,
        });
      } else {
        spawnMessage({
          type: MsgType.DATA_LOCATION,
          text: "",
          title: $scope.ctrlTranslations["splashscreen.selectDataDir"],
        });
      }
    };
