...
app.controller("DebuggingCtrl", [
  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    electron.ipcRenderer.on(
      "debugging-update-settings",
      function (event, msgData) {
        $timeout(function () {
          $scope.settings = msgData.msg[0];
          if (serverData && serverData.linuxoss)
            $scope.oss = serverData.linuxoss;
        }, 0);
      }
    );
    ...
    function readLog() {
      if ($scope.settings) {
        readLastLines.read(getDebugFile($scope.settings), 18).then((lines) => {
          console.log(lines);
          $timeout(function () {
