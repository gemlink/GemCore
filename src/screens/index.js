  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    $scope.getControllerTranslations = function () {
      translationsAvailable().then(() => {
        $translate(["menubar.titleBtn", "menubar.text1", "menubar.text2"]).then(
          (o) => {
    ...
    $scope.menubarClick = function (data, url, isCheck) {
      ...
      if (
        data == $scope.ScreenType.MASTERNODES ||
        data == $scope.ScreenType.MASTERNODES_CONFIG ||
        data == $scope.ScreenType.MASTERNODES_MAP
      ) {
    ...
    $scope.coinSelect = function (coin) {
      ...
      $scope.detail.text =
        $scope.ctrlTranslations["menubar.text1"] +
        " " +
        coin +
        "? " +
        $scope.ctrlTranslations["menubar.text2"];
...
app.controller("NavBarCtrl", [
  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    electron.ipcRenderer.on("child-update-loading", function (event, msgData) {
      ...
      var startTime =
        serverData.starttime == undefined ? 1511111234 : serverData.starttime;
      $timeout(function () {
        $scope.detail.progress =
          (parseInt(bestTime - startTime) / (currTime - startTime)) * 100;
        $scope.detail.width = $scope.detail.progress.toFixed(2) + "%";
        $scope.detail.synctitle =
          $scope.detail.width + " (" + timeConverter(bestTime) + ")";
        $scope.detail.connections = data.connections;
        $scope.detail.currentblock =
          data.block == undefined
            ? ""
            : ", " +
              $scope.ctrlTranslations["navbar.currentBlock"] +
              ": " +
              data.block;
        $scope.detail.lockstate = data.islocked
          ? $scope.ctrlTranslations["navbar.locked"]
    ...
    electron.ipcRenderer.on("child-update-price", function (event, msgData) {
      $timeout(function () {
        ...
        try {
          ...
          var indexGemlink = data.findIndex(function (e) {
            return e.code.toLowerCase() == "glink";
          });
          var indexVDL = data.findIndex(function (e) {
            return e.code.toLowerCase() == "vdl";
          });
          var indexZEC = data.findIndex(function (e) {
            return e.code.toLowerCase() == "zec";
          });
          var indexZER = data.findIndex(function (e) {
            return e.code.toLowerCase() == "zer";
          });
          ...
          PageTransitionEvent;
          priceandsymbol = $scope.detail.priceandsymbol =
            settings.symbol + $scope.detail.price;
...
app.controller("SiteGridMenuCtrl", [
  "$scope",
  "$http",
  "$timeout",
...
app.controller("MenuBarFooterCtrl", [
  "$scope",
  "$http",
  "$timeout",
  "$translate",
