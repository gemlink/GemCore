  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    $scope.getControllerTranslations = function () {
      translationsAvailable().then(() => {
        $translate(["shieldView.privateAddr"]).then((o) => {
    ...
    function updateTransactions(arg) {
      ...
      $timeout(function () {
        if (
          ...
          $scope.transactions[0].key != data[0].txid ||
          $scope.transactions[0].confirmations != data[0].confirmations
        ) {
          ...
          data.some(function (element) {
            ...
            temp["address"] =
              element.address != undefined
                ? element.address
                : $scope.ctrlTranslations["shieldView.privateAddr"];
            temp["amount"] = Math.abs(element.amount);
            if (element.category != "generate") {
              temp["validated"] =
                element.confirmations >= 5
                  ? "Yes"
                  : "No (" + element.confirmations + ")";
            } else {
              temp["validated"] =
                element.confirmations >= 100
                  ? "Yes"
                  : "Immature (" + element.confirmations + ")";
            }
    ...
    $scope.viewDetail = function (data) {
      ...
      $scope.detail.txurl =
        (serverData.explorer.endsWith("/")
          ? serverData.explorer
          : serverData.explorer + "/") + txhash;
      $("#txModal").modal();
    };
