app.controller("SettingsCtrl", [
  ...
  function ($scope, $http, $timeout, $translate, $rootScope) {
    ...
    $scope.detail.bottype = ["Do not use", "Discord"];
    ...
    $scope.openDaemon = function () {
      ...
      if (process.platform == "win32") {
        dialogOptions = {
          filters: [{ name: serverData.daemon, extensions: ["exe"] }],
          properties: ["openFile"],
        };
      } else if (process.platform == "linux" || process.platform == "darwin") {
        dialogOptions = {
          filters: [{ name: serverData.daemon }],
          properties: ["openFile"],
        };
      }
    ...
    $scope.openCli = function () {
      ...
      if (process.platform == "win32") {
        dialogOptions = {
          filters: [{ name: serverData.cli, extensions: ["exe"] }],
          properties: ["openFile"],
        };
      } else if (process.platform == "linux" || process.platform == "darwin") {
        dialogOptions = {
          filters: [{ name: serverData.cli }],
          properties: ["openFile"],
        };
      }
      dialog.showOpenDialog(dialogOptions, function (fileNames) {
    ...
    $scope.valueChange = function (value) {
      ...
      var index = $scope.detail.price.findIndex(function (e) {
        return e.code == value;
      });
      if (index > -1) {
    ...
    $scope.botTypeChange = function () {
      ...
      if ($scope.detail.bot == "Discord" || $scope.detail.bot == "Telegram") {
        $scope.detail.apiKey =
          settings[$scope.detail.bot] == undefined
            ? ""
            : settings[$scope.detail.bot];
      }
    };
    ...
    function backupWalletTimerFunction(fileName) {
      ...
      stopWallet(function () {
        ...
        if (fs.existsSync(walletLocation + "/wallet.dat")) {
          ...
          spawnMessage(
            MsgType.ALERT,
            $scope.ctrlTranslations["settingsView.operations.walletBackup"] +
              " " +
              fileName,
    ...
    $scope.addPeers = function () {
      spawnMessage(
        ...
        "",
        "",
        $scope.ctrlTranslations["settingsView.operations.addPeers"]
      );
    };
    ...
    $scope.restoreClick = function () {
      ...
      {
        dialogOptions = {
          filters: [{ name: serverData.daemon, extensions: ["dat"] }],
          properties: ["openFile"],
        };
      }
      ...
      dialog.showOpenDialog(dialogOptions, function (fileNames) {
        if (fileNames === undefined) {
          ...
          setTimeout(
            restoreWalletStatusTimerFunction,
            500,
        ...
        $timeout(function () {
          ...
          if (data.error == null || data.error == undefined) {
            ...
            spawnMessage(
              MsgType.ALERT,
              $scope.ctrlTranslations["global.success2"],
              $scope.ctrlTranslations["global.close"],
    ...
    electron.ipcRenderer.on("child-update-price", function (event, msgData) {
      $timeout(function () {
        ...
        var indexBtc = data.findIndex(function (e) {
          return e.code.toLowerCase() == "btc";
        });
        var indexEur = data.findIndex(function (e) {
          return e.code.toLowerCase() == "eur";
        });
        var indexRub = data.findIndex(function (e) {
          return e.code.toLowerCase() == "rub";
        });
        var indexGbp = data.findIndex(function (e) {
          return e.code.toLowerCase() == "gbp";
        });
