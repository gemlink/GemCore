...
function createEndWindow() {
  ...
  endWindow.loadFile("src/screens/endscreen.html", {
...
function createSplashWindow() {
  ...
  splashWindow.loadFile("src/screens/splashscreen.html", {
    extraHeaders: "pragma: no-cache\n",
  });
...
function createWindow() {
  ...
  mainWindow.on("closed", function (event) {
    ...
    endWindow
      ? endWindow.webContents.send("child-stop-daemon", { msg: arg })
      : "";
...
function createDebuggingWindow() {
  if (!debuggingWindow) {
    ...
    debuggingWindow.loadFile("src/screens/debuggingscreen.html", {
      extraHeaders: "pragma: no-cache\n",
    });
    ...
    debuggingWindow.once("ready-to-show", function () {
      ...
      debuggingWindow
        ? debuggingWindow.webContents.send("debugging-update-settings", {
            msg: arg,
          })
        : "";
    });
...
ipc.on("main-update-settings", function (event, arg) {
  ...
  splashWindow
    ? splashWindow.webContents.send("child-update-settings", { msg: arg })
    : "";
});
